...
class MyBadgesCommand extends Command {
  ...
  messageRun(message) {
    let badges = message.author.flags.toArray().toString();
		...
		badges = badges.replace('EARLY_VERIFIED_BOT_DEVELOPER', 'Early Verified Bot Developer');
		badges = badges.replace('DISCORD_CERTIFIED_MODERATOR', 'Discord Certified Moderator');
		const badgesArray = badges.split(',');
    if (!badgesArray.length) return message.reply('You have no badges');
		else return message.reply(`These are the badges you have: ${badgesArray.toString()}`);
  }
}
