...
class SlowmodeCommand extends Command {
  ...
  async messageRun(message, args) {
    ...
    if (number.value < 0) return message.reply(`The slowmode can\'t be negative`).then(reply => setTimeout(function() { reply.delete(); message.delete() }, 3500));
    if (number.value > 21600) return message.reply(`You may not set the slowmode to more than 6h`).then(reply => setTimeout(function() { reply.delete(); message.delete() }, 3500));
