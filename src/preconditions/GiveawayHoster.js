...
const { giveawayHosterRoles, giveawayChannels, testingServerID } = require('../../config.json');
...
class GiveawayHosterPrecondition extends Precondition {
    ...
    if (this.container.utility.isStaffMember(message.member)) return this.ok();

        let hasRole = giveawayHosterRoles.some((role) => message.member.roles.cache.has(role))
        if(!hasRole) return this.error('User is not a giveaway hoster');

        let inChannel = giveawayChannels.some((channel) => message.channel.id === channel);
